...
export default async function TrailPage({ params }: { params: { trailId: string } }) {
    const supabase = createClient();
    const { data: trail, error } = await supabase
      .from('Trails')
      .select('id, name, length, type, parking, camping, campfire, accessibility, county, berries_mushrooms, sightseeing, picture')
    ...
    return (
      <div>
        ...
        <p>Pilt: {trail.picture && <img src={trail.picture} alt={trail.name} style={{ width: "100%", maxWidth: "600px" }} />}</p>
      </div>
    );
  }
